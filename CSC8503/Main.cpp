...
void TestPushdownAutomata(Window* w) 
{
	...
	while (w->UpdateWindow()) { // be wary of this to not update window twice
...
void TestNetworking() {
	...
	for (int i = 0; i < 100; ++i) 
	{
		StringPacket host_packet = StringPacket("Server says hello! " + std::to_string(i));
		StringPacket client_packet = StringPacket("Client says hello! " + std::to_string(i));

		server->SendGlobalPacket(host_packet);
		
		client->SendPacket(client_packet);
...
int main() 
{
	string title = "CSC8503 Goat game!";
	Window* w = Window::CreateGameWindow(title, 1280, 720);
	...
	Game* g = new CourseworkGameA();
	PushdownMachine gameController(new GameIntroScreen(g));
	...
	while (w->UpdateWindow() ){
		...
		w->SetTitle(title + " FPS: " + std::to_string(floor(1.0f / dt)));

		if (!gameController.Update(dt))
		{
			break;
		}
	}
