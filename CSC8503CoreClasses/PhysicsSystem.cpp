...
Loads the layer mask combinations that should not collide.

This approach suffers from 'Unity-style' layers and tags being kind of combined in this codebase, 
since it ends up generating a lot of really specific pairings that logically should never collide. 
Making layers less specific by actually using tags would probably help increase performance

Still, checking these masks should still be quicker than doing collision detection, 
especially when it comes to more complex collision volumes.

I'm sure there are still more pairings that were missed

*/
void PhysicsSystem::LoadSkippedCollisions() {
	skippedCollisionMasks.push_back(1 << (int)GameLayers::GL_WALL); // walls are probably the most common object in the game
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_WALL | 1 << (int)GameLayers::GL_DOOR);
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_WALL | 1 << (int)GameLayers::GL_COLLECTABLE);

	skippedCollisionMasks.push_back(1 << (int)GameLayers::GL_COLLECTABLE); // then the score spheres
	skippedCollisionMasks.push_back(1 << (int)GameLayers::GL_COLLECTABLE | 1 << (int)GameLayers::GL_GRAPPLE);
	skippedCollisionMasks.push_back(1 << (int)GameLayers::GL_COLLECTABLE | 1 << (int)GameLayers::GL_KEY);

	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_GOOSE | 1 << (int)GameLayers::GL_COLLECTABLE);
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_GOOSE | 1 << (int)GameLayers::GL_KEY);
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_GOOSE | 1 << (int)GameLayers::GL_GRAPPLE);

	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_ENEMY | 1 << (int)GameLayers::GL_KEY);
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_ENEMY | 1 << (int)GameLayers::GL_GRAPPLE);


	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_GRAPPLE | 1 << (int)GameLayers::GL_KEY);
	
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_KEY);
	skippedCollisionMasks.push_back( 1 << (int)GameLayers::GL_GRAPPLE);
}
...
a particular pair will only be added once, so objec+-ts colliding for
...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; ++i)
	{
		...
		for (auto j = i + 1; j != last; ++j)
		{
			...
			if (CollisionMaskCheck(*i, *j) && CollisionDetection::ObjectIntersection(*i, *j, info))
			{
...
{
	if (a.GetBoundingVolume()->is_trigger || b.GetBoundingVolume()->is_trigger)
	{
		return; // no colision resolution on triggers
	}
	...
	//physA->AddForceAtPosition(Vector3::Cross(relativeA, -fullImpulse), relativeA);
	//physB->AddForceAtPosition(Vector3::Cross(relativeB, fullImpulse), relativeB);
}
		...
		[&](std::list <QuadTreeEntry <GameObject*>>& data) 
		{
			...
			for (auto i = data.begin(); i != data.end(); ++i) {
				for (auto j = std::next(i); j != data.end(); ++j) {
					...
					if (!CollisionMaskCheck((*i).object, (*j).object)) continue;
...
bool PhysicsSystem::CollisionMaskCheck(GameObject* a, GameObject* b)
{
	uint32_t mask = a->GetLayerMask() | b->GetLayerMask();
	for (uint32_t temp : skippedCollisionMasks)
	{
		if (temp == mask)
		{
			return false;
